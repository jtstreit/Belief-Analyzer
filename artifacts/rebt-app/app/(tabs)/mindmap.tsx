  ...
  return (
    <View style={styles.confRow}>
      <View style={[styles.confTrack, { backgroundColor: colors.muted }]}>
        <Animated.View
          style={[styles.confFill, bar, { backgroundColor: colors.primary }]}
        />
      </View>
  ...
  return (
    <View style={styles.layerHeader}>
      <View style={[styles.layerBadge, { backgroundColor: colors.accent }]}>
        <Text
          style={[styles.layerBadgeText, { color: colors.accentForeground }]}
        >
          {number}
        </Text>
      </View>
...
export default function MindMapScreen() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      const sync = await syncLifeOps();
      const refreshed = await refetch();
      if (sync.ingested > 0 || (refreshed.data?.unprocessedCount ?? 0) > 0) {
        await runAnalysis();
      }
      ...
      setAnalysisError(
        "LifeOps scan or analysis failed. Existing insights are safe — tap Scan & Analyse to retry.",
      );
    } finally {
  ...
  const handleDismissSchema = useCallback(
    async (id: number) => {
      try {
        ...
        setAnalysisError(
          "Could not dismiss the schema. Check your connection and try again.",
        );
      }
    },
  ...
  // Every Insights visit first asks LifeOps for new protected signals, then runs
  // Opus only when there is something unprocessed. The debounce prevents rapid
  // tab switches from spawning duplicate requests.
  useFocusEffect(
    useCallback(() => {
    ...
    >
      ...
      <View style={styles.header}>
        <View>
          ...
          {isAnalyzing ? (
            <Text style={[styles.subtitle, { color: colors.primary }]}>
              Scanning LifeOps, then analysing with Opus 4.8…
            </Text>
        ...
        <View style={styles.headerActions}>
          ...
          <TouchableOpacity
            ...
            accessibilityLabel={
              autoAnalyse ? "Auto-analyse on" : "Auto-analyse off"
            }
      ...
      {analysisError ? (
        <Animated.View
          ...
          style={[
            styles.errorBanner,
            { backgroundColor: "#B9404022", borderColor: "#B9404066" },
          ]}
        >
        ...
        {!hasAnyData && !isLoading ? (
          ...
          >
            ...
            <Text style={[styles.emptyBody, { color: colors.mutedForeground }]}>
              Add mood check-ins or thought entries — your map updates
          ...
          <>
            ...
            >
              ...
              {(map?.automaticThoughts?.length ?? 0) === 0 ? (
                <Text
                  style={[styles.emptyLayer, { color: colors.mutedForeground }]}
                >
                ...
                map!.automaticThoughts.slice(0, 6).map((t) => (
                  ...
                  >
                    ...
                    <View style={styles.metaRow}>
                      {t.emotion && (
                        <Text
                          style={[styles.emotion, { color: colors.primary }]}
                        >
                          {t.emotion}
                        </Text>
                      )}
            ...
            >
              ...
              {(map?.coreSchemas?.length ?? 0) === 0 ? (
                <Text
                  style={[styles.emptyLayer, { color: colors.mutedForeground }]}
                >
                  Core schemas emerge after multiple belief patterns are
                  identified.
                </Text>
              ) : (
                map!.coreSchemas.map((s) => {
                  const domainCfg =
                    DOMAIN_CONFIG[s.domain] ?? DOMAIN_CONFIG.other!;
                  return (
                    <View
                      key={s.id}
                      style={[
